...
const EditProfile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      setLoading(true);
      try {
          const { data } = await axios.get(`https://alok-backend.onrender.com/api/profiles/${id}`);
          setFormData({
              name: data.name,
              email: data.email,
              bio: data.bio,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.patch(`https://alok-backend.onrender.com/api/profiles/${id}`, updateData);
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-3">
          ...
          <select id="seeking" name="seeking" value={formData.seeking} onChange={handleChange} className="form-select">
            ...
            <option value="Yes">Internship</option>
            <option value="No">Remote</option>
            <option value="No">Full Time Position</option>
