...
const ProfileList = () => {
  ...
  const fetchProfiles = async () => {
    try {
      const url = searchTerm.trim()
        ? `https://alok-backend.onrender.com/api/profiles/search?search=${encodeURIComponent(searchTerm)}&sort=${sortKey}`
        : `https://alok-backend.onrender.com/api/profiles?sort=${sortKey}`;
  ...
  //   try {
  //     await axios.delete(`https://alok-backend.onrender.com/api/profiles/${profileId}`);
  //   } catch (error) {
  ...
  return (
    <div className="container mt-3">
      <form className="row mb-3">
        ...
        <div className="col" style={{borderWidth:'1px'}}>
